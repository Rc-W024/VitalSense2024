...
elseif SigReproduction==true
    ...
    elseif Rectang_filter==true
        ...
        % set the nominal width
        len_rec_nom=250;
        % width from 50% to 150%
        len=[];
        for i=1:10
            k=[0.5,0.6,0.7,0.8,0.9,1.1,1.2,1.3,1.4,1.5];
            len(i)=fix(len_rec_nom*k(i));
        end
        % rectangular matrix...
        rec50=zeros(100+len(1),1);
        rec50(50:50+len(1))=0.1;
        rec60=zeros(100+len(2),1);
        rec60(50:50+len(2))=0.1;
        rec70=zeros(100+len(3),1);
        rec70(50:50+len(3))=0.1;
        rec80=zeros(100+len(4),1);
        rec80(50:50+len(4))=0.1;
        rec90=zeros(100+len(5),1);
        rec90(50:50+len(5))=0.1;
        nominal=zeros(100+len_rec_nom,1);
        nominal(50:50+len_rec_nom)=0.1;
        rec110=zeros(100+len(6),1);
        rec110(50:50+len(6))=0.1;
        rec120=zeros(100+len(7),1);
        rec120(50:50+len(7))=0.1;
        rec130=zeros(100+len(8),1);
        rec130(50:50+len(8))=0.1;
        rec140=zeros(100+len(9),1);
        rec140(50:50+len(9))=0.1;
        rec150=zeros(100+len(10),1);
        rec150(50:50+len(10))=0.1;
    else
        load('hao_long.mat');
    ...
    elseif Rectang_filter==true
        % 1st MF
        rf50=conv(hsig_lp,rec50);
        rf60=conv(hsig_lp,rec60);
        rf70=conv(hsig_lp,rec70);
        rf80=conv(hsig_lp,rec80);
        rf90=conv(hsig_lp,rec90);
        rf100=conv(hsig_lp,nominal);
        rf110=conv(hsig_lp,rec110);
        rf120=conv(hsig_lp,rec120);
        rf130=conv(hsig_lp,rec130);
        rf140=conv(hsig_lp,rec140);
        rf150=conv(hsig_lp,rec150);
        % signal shifting to eliminate delay
        rf50=circshift(rf50,fix(-length(rec50)/2));
        rf60=circshift(rf60,fix(-length(rec60)/2));
        rf70=circshift(rf70,fix(-length(rec70)/2));
        rf80=circshift(rf80,fix(-length(rec80)/2));
        rf90=circshift(rf90,fix(-length(rec90)/2));
        rf100=circshift(rf100,fix(-length(nominal)/2));
        rf110=circshift(rf110,fix(-length(rec110)/2));
        rf120=circshift(rf120,fix(-length(rec120)/2));
        rf130=circshift(rf130,fix(-length(rec130)/2));
        rf140=circshift(rf140,fix(-length(rec140)/2));
        rf150=circshift(rf150,fix(-length(rec150)/2));
    else
        % 1st MF
        rf_nom1=conv(hsig_lp,nominal);
        rf_sh1=conv(hsig_lp,short);
    ...
    if Rectang_filter==true
        % calcu RMS
        power_rms50=sum(abs(rf50).^2)/length(rf50);
        power_rms60=sum(abs(rf60).^2)/length(rf60);
        power_rms70=sum(abs(rf70).^2)/length(rf70);
        power_rms80=sum(abs(rf80).^2)/length(rf80);
        power_rms90=sum(abs(rf90).^2)/length(rf90);
        power_rms100=sum(abs(rf100).^2)/length(rf100);
        power_rms110=sum(abs(rf110).^2)/length(rf110);
        power_rms120=sum(abs(rf120).^2)/length(rf120);
        power_rms130=sum(abs(rf130).^2)/length(rf130);
        power_rms140=sum(abs(rf140).^2)/length(rf140);
        power_rms150=sum(abs(rf150).^2)/length(rf150);
        % peaks detection
        [amp50,locs50]=findpeaks(rf50,1:length(rf50),'MinPeakProminence',power_rms50);
        [amp60,locs60]=findpeaks(rf60,1:length(rf60),'MinPeakProminence',power_rms60);
        [amp70,locs70]=findpeaks(rf70,1:length(rf70),'MinPeakProminence',power_rms70);
        [amp80,locs80]=findpeaks(rf80,1:length(rf80),'MinPeakProminence',power_rms80);
        [amp90,locs90]=findpeaks(rf90,1:length(rf90),'MinPeakProminence',power_rms90);
        [amp100,locs100]=findpeaks(rf100,1:length(rf100),'MinPeakProminence',power_rms100);
        [amp110,locs110]=findpeaks(rf110,1:length(rf110),'MinPeakProminence',power_rms110);
        [amp120,locs120]=findpeaks(rf120,1:length(rf120),'MinPeakProminence',power_rms120);
        [amp130,locs130]=findpeaks(rf130,1:length(rf130),'MinPeakProminence',power_rms130);
        [amp140,locs140]=findpeaks(rf140,1:length(rf140),'MinPeakProminence',power_rms140);
        [amp150,locs150]=findpeaks(rf150,1:length(rf150),'MinPeakProminence',power_rms150);
        % plot figs...
        figure('Name','Adaptive Matched Filter: RECTANGULAR');
        findpeaks(rf50,1:length(rf50),'MinPeakProminence',power_rms50);
        hold on
        plot(hsig_lp);
        legend('Filtered signal with peaks detected','Extracted cardiac signal')
        % calcu QF
        % index of peaks detection
        IndexPeaks50=(sum(amp50.^2)/length(locs50))/power_rms50;
        IndexPeaks60=(sum(amp60.^2)/length(locs60))/power_rms60;
        IndexPeaks70=(sum(amp70.^2)/length(locs70))/power_rms70;
        IndexPeaks80=(sum(amp80.^2)/length(locs80))/power_rms80;
        IndexPeaks90=(sum(amp90.^2)/length(locs90))/power_rms90;
        IndexPeaks100=(sum(amp100.^2)/length(locs100))/power_rms100;
        IndexPeaks110=(sum(amp110.^2)/length(locs110))/power_rms110;
        IndexPeaks120=(sum(amp120.^2)/length(locs120))/power_rms120;
        IndexPeaks130=(sum(amp130.^2)/length(locs130))/power_rms130;
        IndexPeaks140=(sum(amp140.^2)/length(locs140))/power_rms140;
        IndexPeaks150=(sum(amp150.^2)/length(locs150))/power_rms150;
        % printing...
        IndexN=[50,60,70,80,90,100,110,120,130,140,150];
        IndexP=[IndexPeaks50,IndexPeaks60,IndexPeaks70,IndexPeaks80,IndexPeaks90,IndexPeaks100,IndexPeaks110,IndexPeaks120,IndexPeaks130,IndexPeaks140,IndexPeaks150];
        figure('Name','INDEX OF PEAKS');
        plot(IndexN,IndexP);
        grid on
        xlabel('Width of the filter (%)')
        ylabel('QF')
    else
        % print fig...
        figure;
