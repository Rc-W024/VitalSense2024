...
elseif SigReproduction==true
    ...
    elseif Rectang_filter==true
        ...
        len_rec=80; % width of rectangular wave
        rectang=zeros(100+len_rec,1);
        rectang(50:50+len_rec)=0.1;
        %figure;
        %stem(rectang);
    else
        load('hao_long.mat');
    ...
    elseif Rectang_filter==true
        rf1=conv(hsig_lp,rectang);
        % signal shifting to eliminate delay
        rf=circshift(rf1,-length(rectang)/2);
        figure;
        plot(hsig_lp);
        hold on
        plot(rf);
        legend('Extracted cardiac signal','Filtered signal')
    else
        % 1st MF
        rf_nom1=conv(hsig_lp,nominal);
        rf_sh1=conv(hsig_lp,short);
    ...
    if Rectang_filter==true
        % calcu RMS
        power_rms=sum(abs(rf).^2)/length(rf);
        % peaks detection
        [amp,locs]=findpeaks(rf,1:length(rf),'MinPeakProminence',power_rms);
        figure; % plot of peaks
        findpeaks(rf,1:length(rf),'MinPeakProminence',power_rms);
        % calcu QF
        IndexPeaks=(sum(amp.^2)/length(locs))/power_rms^2; % index of peaks detection
        QF=max(abs(rf).^2)/power_rms;
        % printing...
        fprintf('Index of peaks detection: %.4f\n',IndexPeaks);
