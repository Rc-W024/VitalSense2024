function [phasePlot] = plotPhase(Radar,Digitizer,Sampling,LocFinder,Spec_shift,RMS_Amplitude,DoUnWrapping,titleText,ButtonSignal)
...
decimatedButtonSignal = ButtonSignal(1 : Digitizer.long : end);
%         buttonPlot = plot(Radar.t_frame,decimatedButtonSignal/35000);
%         buttonPlot.Color ='green';
axButton=subplot(length(LocFinder.locs_positive)*2+1,1,length(LocFinder.locs_positive)*2+1);
% = nexttile;
voltageRangeChannelB = 10;
plot(Radar.t_frame,(double(decimatedButtonSignal-2^15)/2^16)*voltageRangeChannelB,'g') % 1000e-3*(double(v-2^15)/2^16)
grid on;
ylabel('Button')
xlabel('t_{obs} (s)')
% axButtonPlot = gca;
% set(axButtonPlot,height,)
linkaxes([Plot(1) axButton],'x')
end
